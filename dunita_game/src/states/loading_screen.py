...
class LoadingScreenState(BaseState):
    ...
    def on_enter(self, mode='new', **kwargs):
        ...
        self.creature_sprites = self._init_creature_sprites(game_data)
    ...
    def _init_creature_sprites(self, game_data):
        """Inicializa los sprites de criaturas"""
        creature_sprites = []
        creatures = game_data.get('creatures', [])
        for c in creatures[:6]:  # Mostrar hasta 6 criaturas
            sprite_key = f"creature_{c['id']}"
            creature_sprites.append({
                'sprite': sprite_key,
                'nombre': c['nombre'],
                'descripcion': c.get('descripcion', ''),
            })
        return creature_sprites
    
    def _init_particles(self):
        """Inicializa las partículas de arena"""
        particles = []
        for _ in range(40):
            particles.append({
    ...
    def _update_animations(self, dt):
        """Actualiza todas las animaciones del loading screen"""
        # Rotar tips de lore
        self.tip_timer += dt
