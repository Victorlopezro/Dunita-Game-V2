...
class AudioManager:
    ...
    def play_bgm(self, state: str):
        ...
        try:
            if self._use_generated:
                self._play_generated_bgm(state)
            else:
                self._play_file_bgm(new_track)
        except Exception as e:
            print(f"BGM play warning: {e}")
    
    def _play_file_bgm(self, track):
        """Reproduce BGM desde archivo"""
    ...
    def _play_generated_bgm(self, state):
        """Reproduce BGM generada proceduralmente"""
        pygame.mixer.stop()
